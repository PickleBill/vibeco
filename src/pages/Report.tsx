    ...
    <div className="min-h-screen bg-background pt-20 pb-16">
      <div className="max-w-3xl mx-auto px-6">
                ...
                return (
                  <motion.div key={section.key} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 + i * 0.05 }}>
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      <h4 className="font-display text-xs font-bold text-foreground uppercase tracking-wide">{section.label}</h4>
                    ...
                    {section.key === "core_features" && Array.isArray(value) ? (
                      ...
                      <p className="font-mono text-sm text-foreground/80 leading-relaxed ml-5">{value as string}</p>
                    )}
                  </motion.div>
                );
              })}
        ...
        {report.lovable_prompt && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.6 }} className="mb-8">
            ...
            <div className="flex justify-center mt-4">
              <button
                onClick={() => {
                  const url = `https://lovable.dev/projects/create#prompt=${encodeURIComponent(report.lovable_prompt!)}`;
                  window.open(url, "_blank");
                }}
                className="flex items-center gap-2 font-mono text-sm bg-primary text-primary-foreground px-6 py-3.5 rounded-sm hover:opacity-90 transition-opacity"
        ...
        <div className="text-center pt-4 border-t border-border/20">
          <p className="font-mono text-xs text-muted-foreground mb-3">Want to simulate your own idea?</p>
          <Link to="/simulate"
