# Prong 2 — Aces × VibeCo: closing the "vibe-coding-on-demand" loop

**Status:** Strategy draft v1 · **Depends on:** Aces v1 shipping (Prong 1)
...
(like Aces), nothing flows back to keep improving it.

**Prong 2 closes that loop.** Aces becomes VibeCo's first *living* portfolio company —
...
```
  RAW IDEA ──▶ VibeCo agents ──▶ Lovable build ──▶ LIVE PRODUCT (Aces)
      ▲                                                     │
      │                                                     ▼
...
| **Project registry** | `project_registry` table + `get_project_registry` | The portfolio ledger Aces registers into. |
...
**Goal:** an Aces user (or Bill) files "make the Ace Card shareable to Instagram Stories,"
and it becomes a scoped, agent-vetted build task.

- **In Aces:** a "Suggest / Report" surface writes to a new `feature_requests` table.
...
**Goal:** living personas for Aces grounded in *real behavior*, not guesses.

- A `persona`-family agent run over real cohort data (the Group Organizer, the Ace Chaser)
  produces and *continuously updates* customer profiles in a `customer_profiles` table.
- These profiles become **context** injected into every other agent run — the
  ...
  *Aces' actual users*. This is the compounding asset.

### Loop D — Expand / distill on the live product
**Goal:** keep evolving Aces with the same agents that vetted the idea — now fed real data.

- `expand-idea` on the live Aces brief → orthogonal new bets (e.g., "Aces for sims/Topgolf,"
  "Aces leagues," "the B2B course contest engine" from the PRD parking lot).
...
- The `auto-evaluate` flywheel scores each expansion → the **Portfolio** page ranks Aces'
  next bets by confidence, the same way it ranks net-new ideas.

---

## Architecture: how Aces plugs in
...
```
                  ...
                  └───────▲───────────────────────▲───────────┘
                          │ invoke_vibeco_agent    │
        ┌─────────────────┴───────┐    ┌───────────┴──────────────┐
        │  VibeCo (idea engine)   │    │  Aces (live product)      │
        │  Supabase project A     │    │  Supabase project B       │
        │  - simulate/expand/...  │    │  - rounds/scores/aces     │
        │                         │    │  - feature_requests       │
        │                         │    │  - feedback_items         │
        └─────────────────────────┘    └───────────────────────────┘
```

- **Separate Supabase project / repo for Aces** (own data, auth, RLS, release cadence,
  and a clean regulatory boundary for any money features).
- **Shared agent mesh via the MCP server** — Aces does *not* re-implement agents. It calls
  them. New task types live in the shared `model-router.ts`.
- **Shared org memory** — decisions made building Aces are searchable from any session
  (`search_decisions`), so the portfolio compounds knowledge instead of fragmenting it.

### Repo recommendation
Spin **Aces into its own repo** before it grows a backend (clean boundary for compliance +
release cadence + a future team). The MCP server can graduate to its own repo too, since
both VibeCo and Aces depend on it. **For now**, everything lives at `/aces` + `/docs` on
...
|---|---|---|
| **P2.0** | Register Aces in `project_registry`; add `aces` project context to the MCP knowledge tools | Aces becomes a first-class portfolio citizen the agents can reason about. Near-zero effort. |
| **P2.1** | `feedback_items` + `feature_requests` tables (with embeddings) + in-Aces "Suggest" surface | The return path can't exist without an inbox. |
...
| **P2.5** | Wire Aces expansions into `auto-evaluate` + surface on the Portfolio page | Closes the loop: the live product feeds the idea engine that spawns the *next* product. |
...
PR, with the rationale written back to shared memory.** Aces is the proof. The Portfolio
